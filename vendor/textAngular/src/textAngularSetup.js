...
// Here we set up the global display defaults, to set your own use a angular $provider#decorator.
    ...
    .value('taSelectableElements', ['a','img', 'video'])
    ...
    .run(['taRegisterTool', '$window', 'taTranslations', 'taSelection', 'Http', '$modal', function(taRegisterTool, $window, taTranslations, taSelection, http, $modal){
        taRegisterTool("html", {
        ...
        taRegisterTool('insertImage', {
            ...
            action: function() {
                var editor = this.$editor();
                $modal.open({
                    templateUrl: 'app/views/template/upload-form.html',
                    controller: function ($scope, $modalInstance) {
                        ...
                        $scope.upload = function(){
                            ...
                            http.upload(formData, function(result){
                                editor.wrapSelection('insertImage', result.datas.url, true);
                                $modalInstance.close();
                            });
                        };
                ...
                $modal.open({
                    ...
                    controller: function ($scope, $modalInstance) {
                        ...
                        $scope.upload = function(){
                            ...
                            formData.append("catalog", "upload");
                            http.upload(formData, function(result){
                                result.datas.url = 'http://fxsd-test.oss-cn-shenzhen.aliyuncs.com/xwar/1054365281561460.mp4';
                                var embed = '<video src="' + result.datas.url + '"  controls="" class="fr-draggable">Your browser does not support HTML5 video.</video>';
